param(
  ...
  [string]$ProbeLog = (Join-Path (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot "..")).Path ".mica\gemini-probe.jsonl"),
...
$gemini = Get-Command $GeminiCommand -ErrorAction SilentlyContinue | Select-Object -First 1
if (-not $gemini) {
  [Console]::Error.WriteLine("Gemini CLI was not found. Install Gemini CLI first, then rerun this probe.")
  exit 2
}
...
"@
  ...
  Write-Host ""
  Write-Host "Mica Gemini CLI probe summary:"
