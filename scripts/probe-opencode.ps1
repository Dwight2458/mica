param(
  ...
  [string]$ProbeLog = (Join-Path (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot "..")).Path ".mica\opencode-probe.jsonl"),
...
if (-not $opencode) {
  [Console]::Error.WriteLine("OpenCode CLI was not found. Install opencode first, then rerun this probe.")
  exit 2
}
