...
def test_runs_page_exposes_interactive_agent_run_form() -> None:
    ...
    assert "Advanced: Execute Docker Command" in source
...
def test_runs_page_does_not_poll_forever_after_runs_finish() -> None:
    ...
    assert 'if (selectedRunStatus !== "started")' in source
    assert "window.setInterval(() => void loadCommands(), 2000)" in source


def test_runs_page_separates_runtime_internal_trace_events() -> None:
    repo_root = Path(__file__).resolve().parents[3]
    source = (repo_root / "apps" / "web" / "src" / "app" / "runs" / "page.tsx").read_text(encoding="utf-8")

    assert "showRuntimeInternalTrace" in source
    assert "commandOriginById" in source
    assert "eventCommandOrigin(event)" in source
    assert '!== "runtime_internal"' in source
    assert "Show {hiddenRuntimeTraceCount} runtime internal events" in source
